...
function MyDashboardDetailPage({ params }: Route.ComponentProps) {
  const { workspaceSlug, dashboardId } = params;
  const { getDashboardById, fetchDashboardDetails } = useDashboards();

  const { isLoading } = useSWR(`my-dashboard-details-${workspaceSlug}-${dashboardId}`, () =>
...
export default observer(MyDashboardDetailPage);
