...
export const IssueTargetDateActivity = observer(function IssueTargetDateActivity(props: TIssueTargetDateActivity) {
  ...
  return (
    ...
    >
      <>
        {activity.new_value ? t("issue_activity.target_date.set") : t("issue_activity.target_date.removed")}
        {activity.new_value && (
          <>
            <span className="font-medium text-primary">{renderFormattedDate(activity.new_value)}</span>
          </>
        )}
        {showIssue && (activity.new_value ? t("issue_activity.target_date.for") : t("issue_activity.target_date.from"))}
